...
# ==========================
# MÓDULO 1 E 2 - IMPORTAÇÃO E ARMAZENAMENTO
# ==========================
...
def filtrar_valor_minimo(dados):
    ...
    print(
        "Valor mínimo:",
        valor,
        nome_unidade("pontos_de_indice") if variavel == "numero_indice" else "%"
    )
    print("==============================================================\n")
...
def mostrar_estatisticas(dados):
    while True:
        ...
        if opcao == "1":
            ...
            print(f"Quantidade total de registros no CSV: {len(dados)}")
...
def gerar_relatorio(dados, caminho_saida="relatorio_turismo.txt"):
    ...
    maiores = ranking_territorios(dados, 5, True, "numero_indice")
    menores = ranking_territorios(dados, 5, False, "numero_indice")
    ...
    linhas.append("\n3. RANKING - TOP 5 TERRITÓRIOS COM MAIOR MÉDIA")
    linhas.append("-" * 60)
    ...
    linhas.append("\n4. RANKING - TOP 5 TERRITÓRIOS COM MENOR MÉDIA")
    linhas.append("-" * 60)
    ...
    if resultado_variacao:
        valor_inicial, valor_final, variacao = resultado_variacao
        tendencia = "cresceu" if variacao > 0 else "caiu"

        linhas.append(f"Valor inicial do índice : {valor_inicial:.2f} pontos")
        linhas.append(f"Valor final do índice   : {valor_final:.2f} pontos")
        linhas.append(
            f"O índice do Brasil {tendencia} "
            f"{abs(variacao):.2f}% no período analisado."
        )
